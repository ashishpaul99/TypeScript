...
// 5. Assertions
type One=string;
type Two=string |number; //type Two = string | number
type Three = 'hello';
...
// eg-2:
let d=<One>'world'; //d is gone have value which is type string

let e=<string | number> 'world'; //string is allowed
...
const add:number=addOrConcat(2,2,'add') as number;
const concat:string=addOrConcat(2,2,'concat') as string;
...
const nextImg =<HTMLImageElement> document.getElementById('#img') ; //this will not work in tsk file for react.
nextImg.src;
